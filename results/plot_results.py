...
def D2plot():
    ...
    n2 =  np.array([256, 1024, 4096, 16384, 65536,262144])
    n1 = np.array(n)
    
    plt.subplot(221)
    plt.loglog(n2, H[::4] ,"o-", label="$\mathcal{H}$-matrix")
    plt.loglog(n, D[::4], "o-", label="full matrix")
    plt.loglog(n2, D[0]*(n2/256)**2,"--",label="$\mathcal{O}(N^2)$")
    plt.legend()
    plt.ylabel("Time (sec)")
    plt.title("Construction")
    plt.subplot(222)
    plt.loglog(n2, H[1::4], "o-", label="$\mathcal{H}$-matrix")
    plt.loglog(n, D[1::4], "o-", label="full matrix")
    plt.loglog(n2, D[1]*(n2/256)**2,"--",label="$\mathcal{O}(N^2)$")
    plt.legend()
    plt.ylabel("Time (sec)")
    plt.title("MatVec")
    plt.subplot(223)
    plt.loglog(n2, H[2::4], "o-", label="$\mathcal{H}$-matrix")
    plt.loglog(n, D[2::4], "o-", label="full matrix")
    plt.loglog(n2, D[2]*(n2/256)**3,"--",label="$\mathcal{O}(N^3)$")
    plt.legend()
    plt.xlabel("N")
    plt.ylabel("Time (sec)")
    plt.title("LU")
    plt.subplot(224)
    plt.loglog(n2, H[3::4], "o-", label="$\mathcal{H}$-matrix")
    plt.loglog(n, D[3::4], "o-", label="full matrix")
    plt.loglog(n2, D[3]*(n2/256)**2,"--",label="$\mathcal{O}(N^2)$")
    plt.legend()
    plt.xlabel("N")
    plt.ylabel("Time (sec)")
    plt.title("Solve")
